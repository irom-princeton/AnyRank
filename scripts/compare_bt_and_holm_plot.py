...
if __name__ == "__main__":
    ...
    for i in range(20):
        ...
        with open(f"outputs/bootstrap_bt_full_metrics/FRF_Heterogeneous/npolicy_5_npairs_{idx}_nredraws_100.json", 'r') as file:
            data = json.load(file)
    ...
    fig.savefig("tmp_correct_ranking_estimates.png", dpi=300)
    ...
    ax2.set_ylim([0., 1.])
    ax2.legend(["Hypotheses Rejected (BT)", "Hypotheses Rejected (Holm)", "Significant Full Ranking (BT)", "Significant Full Ranking (Holm)"], fontsize=16)
    ...
    fig2.savefig("tmp_significant_decisions.png", dpi=300)
