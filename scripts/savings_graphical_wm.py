...
ordered_hypotheses_policy_indices = [(i,j) for i in range(n_policies) for j in range(i+1, n_policies)]
...
if run_new_experiment:
    ...
    for run in tqdm.tqdm(range(n_runs)):
        ...
        rejected_hypotheses, decision_times, p_values, G = graphical_multitest(ordered_hypotheses_policy_indices, policy_data, Nmax, alpha, alpha_per_hypothesis=alpha_per_hypothesis)
