...
def run_graphical_synth(policy_data, real_means, sim_means, init_cond, beta=1., labels=None, subfolder=None, graph_type="soft_masked", methods=None, allow_transitive=True):
    # policy_data = {}
    # policy_data["pi_1"] = np.random.binomial(1, 0.15, 2000)
    # policy_data["pi_2"] = np.random.binomial(1, 0.35, 2000)
    # policy_data["pi_3"] = np.random.binomial(1, 0.55, 2000)
    # policy_data["pi_4"] = np.random.binomial(1, 0.9, 2000)
    
    # real_means = {}
    # real_means["pi_1"] = 0.15
    # real_means["pi_2"] = 0.35
    # real_means["pi_3"] = 0.55
    # real_means["pi_4"] = 0.9
    ...
    # sim_means["pi_3"] = 0.55
    # sim_means["pi_4"] = 0.9
    cfg = ExperimentConfig(alpha=0.05, beta=beta, results_dir=_results_dir(subfolder, f'synth_init_{init_cond}'), graph_type=graph_type)
...
if __name__ == "__main__":
    ...
    for beta in beta_range:
        for init_cond in init_conds_towel:
            ...
            run_graphical_synth(policy_data=policy_data, real_means=real_means, sim_means=sim_means, init_cond=init_cond, beta=beta, labels=plot_policy_names, subfolder=args.subfolder, graph_type=args.graph_type, methods=methods, allow_transitive=args.transitive)
