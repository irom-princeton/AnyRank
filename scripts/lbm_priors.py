...
def plot(show_legend=True):
    ...
    for eval_type in sims:
        ...
        label_y = 0.97 if eval_type == sims[0] else 0.89
        ax.text(0.97, label_y, f"{eval_type}: r = {corr:.2f}",
    ...
    ax.set_xticks(np.arange(min_real - 0.05, max_real + 0.05, 0.05))
    ...
    if show_legend:
        ...
        fig_legend.savefig(f"{output_folder}/carrot_legend.png", dpi=150, bbox_inches=bbox)
        plt.close(fig_legend)

    plt.savefig(f"{output_folder}/corr_carrot.png", dpi=150, bbox_inches='tight')
    plt.close()
